...
char *get_cd_target(char **argv, shell_t *shell){
    if(argv[1] == NULL){
        ...
        if(shell->previous_dir == NULL){write(2, "OLDPWD not set\n", 15); return NULL;}
        return shell->previous_dir; 
    } else {
        return argv[1]; 
    }
}
...
int builtin_cd(int argc, char **argv, shell_t *shell){
    ...
    if(current_dir == NULL){perror(""); return 1;}

    char *target = get_cd_target(argv, shell); 
    if(target == NULL){free(current_dir); return 1;}

    else if(change_directory(target) != 0){
        free(current_dir); 
        return 1; 
    }
    free(shell->previous_dir); 
    shell->previous_dir = current_dir; 
